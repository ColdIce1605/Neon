...
uniform vec2 viewPixelSize;
	...
	void main() {
		...
		lightmapCoordinates = (gl_TextureMatrix[1] * gl_MultiTexCoord0).xy;
	...
	void main() {
		vec4 baseTex = textureLod(texture, textureCoordinates, 0.0) * tint;
		if (baseTex.a < 0.102) discard;
		vec4 lightTex = textureLod(lightmap, lightmapCoordinates, 0.0);
		...
		outNormalId = vec4(encodeNormal(normal) * 0.5 + 0.5, id / 255.0, pack2x8(lightTex.xy));
	}
#endif
